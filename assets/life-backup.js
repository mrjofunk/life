...
function build_sections(data) {
  ...
  for (var d in data.groups) {
    ...
    h2.innerHTML = data.groups[d].title;
    ...
    for (var o in data.groups[d].items) {
      let doo = data.groups[d].items[o];
      ...
      // from
      var date_from = doo.from ? new Date(doo.from) : first_day_on_screen;
      var from = (date_from - first_day_on_screen) / one_day;
      if (unit == "weeks") {
      ...
      // to
      var date_to = doo.to ? new Date(doo.to) : new Date();
      var to = (date_to - first_day_on_screen) / one_day;
      if (unit == "weeks") {
      ...
      var width = dots.eq(to).position().left - left + 10;

      // custom color
      var color = doo.color || colors[color_count];
      //console.log(color);

      // set style
      block.style = "background:#" + color + "; margin-left:" + (left * (1 / scale_factor)) + "px; width:" + (width * (1 / scale_factor)) + "px;";
      ...
      title.innerHTML = "<name>" + doo.title + "</name>";
      block.appendChild(title);

      // info -------------------------------------------

      // from ist entweder definiert oder der geburtstag
      var ifrom = doo.from ? new Date(doo.from) : new Date(data.from);

      // to ist entweder definiert oder heute
      var ito = doo.to ? new Date(doo.to) : new Date();

      // dates
      var infoHTML = "<date>  " + picker.dateObjectsToHtmlValue([doo.from, doo.to], "date") + "</date>";

      // days
      var days = Math.round((ito - ifrom) / one_day);

      // total days
      total_days_of_group += days;
      if (last_to && last_to > ifrom) {
        var u = Math.round((ifrom - last_to) / one_day);
        total_days_of_group += u;
      }
      last_to = ito;
      ...
      style +=
        ...
        "):nth-child(-n+" +
        (to + 1) +
        "):not(.past){ background-color:#" + alpa + "; }";
  ...
  if (el && el.length) {
    ...
    if (el.length) {
      ...
      el.closest('.group').find('> div .block').each(function () {
        var from = parseInt($(this).attr("from"), 10);
        var to = parseInt($(this).attr("to"), 10);
        t.push("#content div:nth-child(n+" + from + "):nth-child(-n+" + to + "){background-color:var(--past-over-section-color)!important;}");
      });

      var color = el.css("background-color");
      var alpa = color.replace('rgb', 'rgba').replace(')', ', .5)');
      var from = parseInt(el.attr("from"), 10) + 1;
      var to = parseInt(el.attr("to"), 10) + 1;

      var style =
        "#content div{background-color:var(--past-color)!important; transition:none;}" +
        t.join('') +
        "#content div:nth-child(n+" +
        from +
        "):nth-child(-n+" +
        ...
        "):nth-child(-n+" +
        to +
        "):not(.past){ background-color:" + alpa + "!important; }";
      styles.html("@media screen{" + style + "}");
      return;
    }
  }
